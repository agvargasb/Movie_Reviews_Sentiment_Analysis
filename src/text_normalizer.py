...
def remove_special_chars_better(text: str, remove_digits: Optional[bool] = False) -> str:
    """
    Remove non-alphanumeric characters from input string.
...
def remove_stopwords(
    ...
    # ¿Este código que objetivo tiene?
    # if is_lower_case:
...
def normalize_corpus(
    ...
    fraction_len_corpus = int(len_corpus / 10)
    i = 0
    ...
    for doc in corpus:
        ...
        if special_char_removal:
            doc = remove_special_chars_better(doc, remove_digits=remove_digits)
        ...
        if i % fraction_len_corpus == 0:
            progress = round((i / len_corpus) * 100, 2)
